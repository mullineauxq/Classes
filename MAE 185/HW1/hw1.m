...
%% Part 5 - Plotting k, Turbulence kinetic energy 

% to begin,create a 3D matrix containing all fluctuation u and v values, at
% each position, in each time step y looping through each time step and
% subtracting the mean velocities

t_steps  = length(u);
u_flux_sqd = zeros(size(u));
v_flux_sqd = zeros(size(v));

for j = 1:t_steps
u_flux_sqd(j,:,:) = squeeze(u(j,:,:)) - u_bar_mean;
v_flux_sqd(j,:,:) = squeeze(v(j,:,:)) - v_bar_mean;
end

% now, in an elementwise manner, square all velocities fluctuations

u_flux_sqd = u_flux_sqd.^2;
v_flux_sqd = v_flux_sqd.^2; 

% now take the time mean of both u flux and v flux squared

u_flux_sqd_mean = squeeze(mean(u_flux_sqd));
v_flux_sqd_mean = squeeze(mean(v_flux_sqd));

% define the array of valyes for turbulence KE k(x,y)
k = 1/2*(u_flux_sqd_mean+v_flux_sqd_mean);

% now plot k over all x and y positions over the experiment in a color map
